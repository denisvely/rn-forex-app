...
const TakeProfitAmount = ({ state, setState }) => {
  ...
  const spinnerMax = currentTrade.isBuy
    ? 1000000
    : (
        (currentlyModifiedOrder.rate - currentlyModifiedOrder.maxTPRate) *
        parseFloat(
          convertUnits(
            currentlyModifiedOrder.volume,
            currentlyModifiedOrder.tradableAssetId,
  ...
  const recalculateProfitLossTPAmount = (value) => {
    if (value) {
    ...
    if (value) {
      if (state.takeProfitAmount) {
        ...
        if (parseFloat(value) < parseFloat(spinnerMin)) {
          recalculateProfitLossTPAmount(parseFloat(spinnerMin));
        } else {
          recalculateProfitLossTPAmount(value);
        }
      } else {
        //   SpinnerOnStart
        if (currentTrade.isBuy) {
